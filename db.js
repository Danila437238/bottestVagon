...
class Data {
  ...
  static getPositionsInRange(start, end, cb) {
    return db.all(`SELECT rowid, * FROM positions LIMIT ?, ?`, [start, end], cb);
  }
  static allPositionsToPuschase(start, end, cb) {
    return db.all("SELECT rowid, * FROM positions WHERE rowid BETWEEN ? AND ?", [start, end], cb);
  }
  ...
  static findIngredientByName(name, cb) {
    return db.get("SELECT rowid, * FROM ingredients WHERE ingredient = ?", name, cb);
  }
